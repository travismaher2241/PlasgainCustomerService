...
const TEST_CATALOGUE: PlasgainProduct[] = [
  ...
  {
    id: "prod-cable-cover",
    name: "Polymeric Cable Cover",
    code: "PCC-150",
    category: "Cable & Civil Protection",
  ...
  {
    id: "prod-plaspole",
    name: "Plaspole Recycled Composite Pole",
    code: "PLASPOLE-6M",
...
describe("Canonical Product Resolution & Package Preflight (P0-12, P0-13, P0-14)", () => {
  it("resolves exact product codes reliably into verified PlasgainProducts (P0-12)", () => {
    const res1 = resolveSingleProduct("50W-INTENSE", TEST_CATALOGUE);
    ...
    const res2 = resolveSingleProduct("PBS-75 / PBS-125", TEST_CATALOGUE);
  ...
  it("resolves approved product aliases and common drawing text variants", () => {
    const alias1 = resolveSingleProduct("Pro Blade Solar 75W Area Luminaire", TEST_CATALOGUE);
    expect(["EXACT_MATCH", "ALIAS_MATCH"]).toContain(alias1.status);
    expect(alias1.product?.id).toBe("prod-pro-blade");

    const alias2 = resolveSingleProduct("Polymeric Cable Protection Roll 150mm", TEST_CATALOGUE);
    expect(["EXACT_MATCH", "ALIAS_MATCH"]).toContain(alias2.status);
    expect(alias2.product?.id).toBe("prod-cable-cover");

    const alias3 = resolveSingleProduct("6m Plaspole Composite Light Pole", TEST_CATALOGUE);
    expect(["EXACT_MATCH", "ALIAS_MATCH"]).toContain(alias3.status);
    expect(alias3.product?.id).toBe("prod-plaspole");
  });

  it("handles empty clean slate catalogue safely without crashing", () => {
    const res = resolveSingleProduct("50W-INTENSE", []);
    expect(res.status).toBe("UNMATCHED");
    expect(res.confidence).toBe(0);
    expect(res.suggestedMatches).toEqual([]);
  });
  ...
  it("preflights deal products and correctly reports match counters", () => {
    ...
    const preflight = preflightProductPackage(dealItems, TEST_CATALOGUE);
  ...
  it("flags unverified or proprietary customer line items for manual mapping (P0-14)", () => {
    ...
    const preflight = preflightProductPackage(mixedItems, TEST_CATALOGUE);
    ...
    const preflightAfterMapping = preflightProductPackage(mixedItems, TEST_CATALOGUE, {
