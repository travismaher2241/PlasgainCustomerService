...
describe('CRMQuickLogModal Component', () => {
  it('opens and updates activity title dynamically when switching activity types', async () => {
    ...
    const titleInput = screen.getByDisplayValue(/Call with City of Moreton Bay/i);
    ...
    expect(screen.getByDisplayValue(/Email sent to City of Moreton Bay/i)).toBeInTheDocument();

    // Switch to Meeting
    const meetingButton = screen.getByRole('button', { name: /Meeting/i });
    fireEvent.click(meetingButton);
    expect(screen.getByDisplayValue(/Meeting with City of Moreton Bay/i)).toBeInTheDocument();
  });
});
